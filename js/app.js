...
function renderStandbyReports(afterSchedule=false){
  ...
  const rows = state.teachers.map(t=>{
    ...
    return afterSchedule
      ? `<tr><td>${escapeHtml(t.name)}</td><td>${escapeHtml(t.spec)}</td><td>${des}</td><td>${plc}</td><td>${rem}</td><td>${status}</td></tr>`
      : `<tr><td>${escapeHtml(t.name)}</td><td>${escapeHtml(t.spec)}</td><td>${des}</td></tr>`;
  }).join("");

  const summary = afterSchedule
    ? `<table><thead><tr><th>المعلم</th><th>التخصص</th><th>مطلوب انتظار</th><th>مجدول</th><th>متبقي</th><th>الحالة</th></tr></thead><tbody>${rows}</tbody></table>`
    : `<table><thead><tr><th>المعلم</th><th>التخصص</th><th>انتظار مطلوب</th></tr></thead><tbody>${rows}</tbody></table>`;

  if(!afterSchedule){ wrap.innerHTML = summary; return; }

  // Standby matrix: teacher rows, day/period columns (for printing)
  const lens = state.schedule?.globalLens || [7,7,7,7,7];
  const tSched = state.schedule?.teachers || {};
  const head1 = `<tr><th rowspan="2">م</th><th rowspan="2">اسم المعلم</th>${DAYS.map(d=>`<th colspan="7">${escapeHtml(d)}</th>`).join("")}</tr>`;
  const head2 = `<tr>${DAYS.map(()=>Array.from({length:7},(_,i)=>`<th>${i+1}</th>`).join("")).join("")}</tr>`;

  const body = state.teachers.map((t,ti)=>{
    const grid = tSched[t.id]?.grid || Array.from({length:5},()=>Array(7).fill(""));
    const cells = DAYS.map((_,di)=>Array.from({length:7},(_,pi)=>{
      if(pi>=lens[di]) return `<td class="off"></td>`;
      return (grid[di][pi]===SUBJECTS.STANDBY) ? `<td class="standby">${SUBJECTS.STANDBY}</td>` : `<td></td>`;
    }).join("")).join("");
    return `<tr><td>${ti+1}</td><td class="teacher-name">${escapeHtml(t.name)}</td>${cells}</tr>`;
  }).join("");

  const matrix = `
    <div class="h3">جدول الانتظار الأسبوعي (شامل)</div>
    <table class="standby-matrix">
      <thead>${head1}${head2}</thead>
      <tbody>${body}</tbody>
    </table>
  `;

  wrap.innerHTML = summary + matrix;
}
...
function buildSchedule(){
  ...
  function buildOnce(relaxRules){
    ...
    // iterate sections: prefer the most constrained (more lessons/teachers) then add light randomization
    const secOrder = sections.slice().sort((a,b)=>{
      const aN = (lessonsBySectionBase[a.key]||[]).length;
      const bN = (lessonsBySectionBase[b.key]||[]).length;
      const aT = new Set((lessonsBySectionBase[a.key]||[]).map(x=>x.teacherId||"")).size;
      const bT = new Set((lessonsBySectionBase[b.key]||[]).map(x=>x.teacherId||"")).size;
      return (bN*2 + bT) - (aN*2 + aT);
    });
    // tiny shuffle within near-equal difficulty to avoid systematic bias
    for(let i=0;i<secOrder.length-1;i++){
      if(Math.random()<0.35){ const j=Math.min(secOrder.length-1, i+Math.floor(Math.random()*3)); [secOrder[i],secOrder[j]]=[secOrder[j],secOrder[i]]; }
    }
  ...
  // المرحلة 1: صارمة
  const strictRun = bestOf(80, false);
  // المرحلة 2: (اختياري) تخفيف قيود لتقليل الفراغات
  const relaxedRun = forceFill ? bestOf(160, true) : null;
  ...
  // global limit: no more than 2 standby teachers at the same time slot
  const standbyCount = Array.from({length:5},()=>Array(7).fill(0));
  ...
  for(const t of state.teachers){
    ...
    while(desired>0 && progressed){
      ...
      for(const c of candidates){
        if(desired<=0) break;
        // اختر أول خانة مناسبة مع مراعاة:
        // 1) عدم تتابع الانتظار لنفس المعلم
        // 2) حد أقصى 2 معلمين انتظار في نفس الحصة
        let p;
        while((p = c.slots.shift()) !== undefined){
          if(p>=globalLens[c.d]) continue;
          if(teacherOcc[t.id][c.d][p]!==null) continue;
          if(standbyCount[c.d][p] >= 2) continue;
          const left = p>0 ? teacherOcc[t.id][c.d][p-1] : null;
          const right = p<6 ? teacherOcc[t.id][c.d][p+1] : null;
          if(left===SUBJECTS.STANDBY || right===SUBJECTS.STANDBY) continue;
          break;
        }
        if(p===undefined) continue;
        teacherOcc[t.id][c.d][p]=SUBJECTS.STANDBY;
        standbyCount[c.d][p] += 1;
